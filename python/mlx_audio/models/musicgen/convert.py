...
def convert_musicgen_weights(
    ...
    print(f"Loading MusicGen weights from {pytorch_path}...")
    ...
    for pt_key, pt_tensor in state_dict.items():
        ...
        mlx_key = _map_musicgen_key(pt_key)
        ...
        np_array = _transform_musicgen_weight(pt_key, np_array)
...
def _map_musicgen_key(pt_key: str) -> str | None:
    ...
    # Map decoder layers
    # HF: decoder.model.decoder.layers.{i}.* -> decoder.layers.{i}.*
    mlx_key = re.sub(
    ...
    mlx_key = re.sub(r"\.encoder_attn\.", ".cross_attn.", mlx_key)

    # Map layer norms
    mlx_key = re.sub(r"\.self_attn_layer_norm\.", ".self_attn_layer_norm.", mlx_key)
    mlx_key = re.sub(r"\.encoder_attn_layer_norm\.", ".cross_attn_layer_norm.", mlx_key)
    ...
    mlx_key = re.sub(
        r"^decoder\.model\.decoder\.layer_norm\.",
        "decoder.layer_norm.",
        mlx_key
    )
...
def _transform_musicgen_weight(key: str, np_array: np.ndarray) -> np.ndarray:
    """Transform weight array for MLX format."""
    # Linear layers in PyTorch are [out, in], MLX expects [in, out] for Linear
    # But MLX Linear actually stores [out, in] as well, so no transpose needed
    # for standard Linear layers

    # However, attention in_proj weights might need special handling
    # if they're combined Q/K/V projections
...
def download_and_convert(
    ...
    Args:
        model_name: Model name (musicgen-small, musicgen-medium, musicgen-large, musicgen-melody)
...
def main():
    ...
    parser.add_argument(
        "-m", "--model",
        choices=["musicgen-small", "musicgen-medium", "musicgen-large", "musicgen-melody"],
